...
interface LeftPageProps { product: Product; pageNumber: number; }
const LeftPage = React.forwardRef<HTMLDivElement, LeftPageProps>(({ product }, ref) => (
  <div className="page product-page left-page border border-gray-300 border-r-amber-500 border-r-7" ref={ref}>
    <div className="page-inner">
      <div>
        <div className="left-image-area mt-10 p-2 w-fit border border-green-500 rounded-2xl shadow-lg mt-16 mx-auto">
          <div className="flex justify-center bg-white rounded-xl">
            <img
              src={getProductImageUrl(product, 'url')}
              alt={product.name}
        ...
        <div className="detail-info flex flex-col items-center justify-center text-center">
          <span className="block text-xs uppercase tracking-wide opacity-80 text-black mt-8">
            Crafted by
          </span>
          <span className="block font-semibold text-3xl md:text-4xl text-black">
            {product?.artist_name || 'Artisan'}
          </span>
        </div>
      </div>
...
interface RightPageProps { product: Product; pageNumber: number; onViewMore?: (product: Product) => void; }
const RightPage = React.forwardRef<HTMLDivElement, RightPageProps>(({ product, onViewMore }, ref) => (
  <div className="page product-page right-page flex justify-center items-center" ref={ref}>
    <div className="page-inner relative w-full max-w-[420px] rounded-2xl bg-gradient-to-br from-green-800 via-green-700 to-green-600 p-6 md:p-8 shadow-2xl text-white">
      <div className="details-container flex flex-col justify-center items-center gap-6 text-center h-full">
        ...
        <div className="flex flex-col items-center gap-8 mt-24 sm:mt-32 md:mt-40">
          <div className="price-card rounded-xl bg-green-700 px-8 py-2 sm:px-10">
            <span className="block text-[10px] uppercase tracking-widest opacity-80">
              Price
            </span>
            <div className="mt-1 flex justify-center items-end gap-1 font-bold">
          ...
          {onViewMore && (
            <button
              onClick={() => onViewMore(product)}
              className="w-full max-w-xs rounded-xl bg-white py-3 font-bold text-green-800 transition hover:bg-green-100 hover:-translate-y-0.5"
            >
              View Full Details
            </button>
          )}
...
export default function BookFlip() {
  ...
  const [loading, setLoading] = useState(true);
  const [selectedProduct, setSelectedProduct] = useState<Product | null>(null);
  const [bookSize, setBookSize] = useState({ width: 480, height: 620 });

  const flipBookRef = useRef<FlipBookRef>(null);
  const { user } = useAuthStore();
  const isAdmin = user?.role === 'ADMIN';

  // Responsive book size – smaller on mobile
  useEffect(() => {
    const updateSize = () => {
      ...
      if (isMobile) {
        // Smaller book on mobile → fully visible with margin
        setBookSize({
          width: Math.min(w * 0.78, 360),     // 78% width – feels compact
          height: Math.min(h * 0.65, 520),    // reduced height proportion
        });
      } else {
        setBookSize({
          width: 520,
  ...
  pages.push(<FrontCover key="front-cover" />);
  products.forEach((product, index) => {
    pages.push(<LeftPage key={`left-${product.id}`} product={product} pageNumber={index * 2 + 1} />);
    pages.push(
      <RightPage
        key={`right-${product.id}`}
        product={product}
  ...
  return (
    <>
      <div className="catalogue-wrapper">
        ...
        {/* <div className="catalogue-footer">
          <span className="hint-text">💡 Swipe or use arrows to flip pages</span>
        </div> */}
      </div>
      ...
      {/* ================= STYLES ================= */}
      <style jsx global>{`
        .catalogue-book { overflow: hidden !important; }
        .stf__wrapper, .stf__parent { overflow: hidden !important; }
        ...
        .cover-spine {
          ...
          width: 12px;
          background: linear-gradient(to right, rgba(0,0,0,0.4), rgba(0,0,0,0.1) 40%, rgba(255,255,255,0.05) 60%, rgba(0,0,0,0.3));
        }

        .cover-spine.right {
          right: auto; left: 0;
          background: linear-gradient(to left, rgba(0,0,0,0.4), rgba(0,0,0,0.1) 40%, rgba(255,255,255,0.05) 60%, rgba(0,0,0,0.3));
        }
        ...
        /* LEFT & RIGHT PAGE ADJUSTMENTS */
        .left-image-area { margin-top: 2rem !important; }
        .product-img { max-width: 100%; object-fit: contain; }
        ...
        @media (max-width: 640px) {
          ...
          .mt-40, .mt-32 { margin-top: 4rem !important; }
          .price-card { padding: 10px 16px !important; }
          .price-card span.text-xl { font-size: 1.4rem !important; }
        }
      `}</style>
      ...
      <style jsx>{`
        ...
        @media (max-width: 640px) {
          .book-container { gap: 10px; padding: 12px 6px; }
          .nav-btn { padding: 10px 14px; }
          .nav-btn svg { width: 20px; height: 20px; }
          .catalogue-wrapper { padding: 10px 8px; }
        }
      `}</style>
    </>
  );
...
/* ================= LOADING STYLES ================= */
const loadingStyles = `
  .catalogue-loading, .catalogue-empty {
    display: flex;
    flex-direction: column;
    align-items: center;
    justify-content: center;
  ...
  .loading-pages {
    position: absolute;
    right: 4px; top: 4px; bottom: 4px; left: 8px;
    display: flex;
    flex-direction: column;
    gap: 2px;
  }
  .loading-page { flex: 1; background: #f0fdf4; border-radius: 0 3px 3px 0; }
  @keyframes openBook {
    0%, 100% { transform: rotateY(0deg); }
    50% { transform: rotateY(-25deg); }
  }
  .loading-text { font-size: 1rem; color: #065f46; font-weight: 500; }
  .empty-icon { font-size: 3.5rem; opacity: 0.5; }
  .empty-text { font-size: 1.1rem; color: #6b7280; }
`;
